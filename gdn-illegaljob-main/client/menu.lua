...
RegisterNetEvent("gdn-illegaljob:anthertray", function()
    if QBCore.Functions.GetPlayerData().job.name =='gallery' and QBCore.Functions.GetPlayerData().job.onduty then
        TriggerEvent("inventory:client:SetCurrentStash", "trayy1")
        TriggerServerEvent("inventory:server:OpenInventory", "stash", "trayy1", {
            maxweight = 1000000,
            slots = 10,
        })
    end
...
Citizen.CreateThread(function()
    if QBCore.Functions.GetPlayerData().job.name =='gallery' and QBCore.Functions.GetPlayerData().job.onduty then
        ...
        exports['qb-target']:AddBoxZone("crafting1", vector3(-1531.55, -400.22, 35.63), 2, 1, {
            name="crafting1",
            heading= 320,
            --debugPoly=true,
            minZ= 32.83,
            ...
            options = {
                {
                    event = "gdn-illegaljob:anthertray",
                    icon = "fas fa-tray",
                    label = "Tray2",
                },
            },
